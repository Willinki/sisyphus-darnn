...
from local_exp.utils.learning_rate import make_lr_map
from local_exp.logging.logging_utils import Logger
from local_exp.utils.symmetrize_wback import symmetrize_w
...
def main(cfg: DictConfig):
    ...
    lr_map = make_lr_map(
        orchestrator, (cfg.optimizer.special.to_idx, cfg.optimizer.special.from_idx)
    )
    optimizer = optax.multi_transform(
        {
            "default": optax.adam(learning_rate=cfg.optimizer.learning_rate),
            "special": optax.sgd(learning_rate=cfg.optimizer.special.learning_rate),
        },
        lr_map,
    )
    ...
    for epoch in range(1, cfg.epochs):
        ...
        # Training epoch
        orchestrator = symmetrize_w(orchestrator, (2, 1))
        trainer.orchestrator = orchestrator
